...
jest.mock('amqplib');
jest.mock('@theshreyashguy/coffee-shared');
...
describe('Email Consumer', () => {
  ...
  describe('consumeAuthNotificationMessages method', () => {
    it('should be called', async () => {
      ...
      await consumeAuthNotificationMessages(connectionChannel!);
      expect(connectionChannel!.assertExchange).toHaveBeenCalledWith('coffee-push-notification', 'direct');
      expect(connectionChannel!.assertQueue).toHaveBeenCalledTimes(1);
      expect(connectionChannel!.consume).toHaveBeenCalledTimes(1);
      expect(connectionChannel!.bindQueue).toHaveBeenCalledWith('auth-push-queue', 'coffee-email-notification', 'auth-email');
    });
  });

  describe('consumeOrderNotificationMessages method', () => {
    it('should be called', async () => {
      const channel = {
        assertExchange: jest.fn(),
        publish: jest.fn(),
      ...
      jest.spyOn(connection, 'createConnection').mockReturnValue(channel as never);
      const connectionChannel: amqp.Channel | undefined = await connection.createConnection();
      await consumeOrderNotificationMessages(connectionChannel!);
